...
**阶段 1：固定初始标签目录与扁平标签接口**：
  - 固定 15 个扁平初始标签，使用英文稳定 ID 持久化，并通过目录映射中文显示名称。
  - 新增 Application 层标签目录查询、添加和移除接口，重复操作保持幂等，非初始标签明确报错。
  - 保留旧数据库和旧分类标签结构，不接入标签目录配置 UI；同步阶段 1 规格、架构、简化方案、标签基线和路线图。
  - Ruff 检查、格式检查和完整测试通过（1682 passed、4 skipped）。
## [0.52.3] - 2026-09-19

**阶段 1：临时目录样本验证**：
  - 补充空标签、不可读根目录/条目、损坏元数据和原子替换失败测试。
  - 验证中文路径、规范化路径、索引重建和失败时保留已有 `arlo.ini`。
  - Ruff 检查、格式检查和完整测试通过（1679 passed、4 skipped）。

## [0.52.2] - 2026-09-19

**阶段 1：运行时边界收敛与验证补强**：
  - 将阶段 1 收敛为现役资产根目录、`arlo.ini`、一级只读扫描和可重建运行时索引；旧目录迁移改为后续独立候选。
  - 明确作者/来源暂不进入阶段 1 标签模型，并同步规格、架构和标签基线文档。
  - 修复重复标签写入时的应用层异常转换，补充中文路径、规范化路径和失败场景测试。
  - Ruff 检查、格式检查和完整测试通过（1675 passed、4 skipped）。

## [0.52.1] - 2026-09-19

**阶段 1：运行时资产库内存索引**：
  - 新增由文件系统和 `arlo.ini` 扫描结果重建的运行时内存索引。
  - 支持规范化路径查找、标签交集筛选和文件夹名称/标签搜索。
  - 保持新资产库路径与旧数据库、目录树、操作历史和多根目录服务隔离。

## [0.52.0] - 2026-09-19

**阶段 1：本地元数据与一级资产库扫描基线**：
  - 新增 `arlo.ini` 最小标签格式、稳定标签 ID 校验、显示名称分离和原子写入。
  - 新增不依赖数据库的一级资产库只读扫描，识别内容单元、松散文件、隐藏/系统条目和元数据错误。
  - 新增应用层标签写入边界与阶段 1 专项测试；全量测试 1671 passed、4 skipped。
  - 本批次不接入 UI、不执行真实资源迁移，旧数据库和旧扫描链路暂保留待后续隔离。

## [0.51.6] - 2026-09-18

**阶段 0：人工验收收尾**：
  - 记录真实启动验收：窗口正常显示，已有标签、缩略图、设置、日志和内容均保持完整。
  - 阶段 0 的改名、入口、数据目录和活动文档基线完成收口。
