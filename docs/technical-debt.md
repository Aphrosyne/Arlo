...
>
> 文中的旧项目名仅出现在历史问题、兼容字段或历史路径记录中；当前产品名和入口以 Arlo 及活动路线图为准。
...
- **位置**: [models.py](../src/domain/models.py) `OperationHistory.__post_init__`
...
- **位置**: [scan_service.py](../src/application/scan_service.py) `_persist_scan_result`
...
- **位置**: [main_window.py](../src/app/main_window.py) `_refresh_content_list` → [content_service.py](../src/application/content_service.py) `list_directory_entries`
...
- **位置**: [content_unit.py](../src/infrastructure/repositories/content_unit.py) `list_by_path_prefix`
- **修复**: 转义 `prefix + sep` 中的 `%`、`_`、`\`，使用 `ESCAPE '\\'` 子句。

### TD-H7: list_by_path_prefix 在分隔符分歧场景下漏匹配子路径 ✅ 已修复（v0.16.0）

- **位置**: [content_unit.py](../src/infrastructure/repositories/content_unit.py)
...
- **位置**: [quick_insert_service.py](../src/application/quick_insert_service.py) `_sync_folder_cache` / [mod_group_service.py](../src/application/mod_group_service.py) `create_mod_group` 步骤 1b
...
- **位置**: [scan_service.py](../src/application/scan_service.py) `ScanSummary.success`
...
- **位置**: [scan_service.py](../src/application/scan_service.py) 第 176 行
...
- **位置**: [errors.py](../src/application/errors.py) 第 25-26 行
...
- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) 第 92 行
...
- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) `list_root_nodes` / `_list_children_of_managed_root` / `_get_managed_root_node`
...
- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) 第 158-160 行
...
- **位置**: [managed_root_service.py](../src/application/managed_root_service.py) / [scan_service.py](../src/application/scan_service.py)
...
- **位置**: [content_service.py](../src/application/content_service.py) 第 119 行 / [scan_service.py](../src/application/scan_service.py) 第 219 行
...
- **位置**: [content_service.py](../src/application/content_service.py) 第 105-110 行
...
- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) 第 317 行
- **问题**: `fc_root` 参数无类型标注，违反 AGENTS.md 类型标注要求。
- **建议**: 改为 `fc_root: FolderCache`。

### TD-M11: _commit 数据库提交失败无 UI 反馈 ✅ 已修复（Stage 5 Task 0）

- **位置**: [main_window.py](../src/app/main_window.py) `_commit`
...
- **位置**: [main_window.py](../src/app/main_window.py) 第 431-441 行
- **问题**: 异常时 `entries = []`，显示"该目录为空"，用户无法区分错误与真空目录。
- **建议**: 异常时设置不同的提示文本。

### TD-M13: scan_progress 信号声明但不更新进度

- **位置**: [scan_worker.py](../src/app/scan_worker.py) 第 51 行
...
- **位置**: [main_window.py](../src/app/main_window.py) `_refresh_content_list`
- **问题**: 每次目录树选中都重新读取文件系统，用户在目录间切换时反复执行相同 I/O。
- **建议**: 引入带目录 mtime 失效的缓存。

### TD-M17: with get_connection(...) as conn: 不关闭连接

- **位置**: [db.py](../src/infrastructure/db.py) `init_db` / [test_db.py](../tests/test_db.py)（8 处）
...
- **位置**: [test_managed_root_repository.py](../tests/test_managed_root_repository.py) `test_delete_commits_without_explicit_commit` / [test_managed_root_service.py](../tests/test_managed_root_service.py) `test_remove_root_persists_without_explicit_commit`
...
- **位置**: [folder_cache.py](../src/infrastructure/repositories/folder_cache.py) `upsert_mtime`
...
- **位置**: [test_content_service.py](../tests/test_content_service.py) 第 26-34 行
...
- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) `__post_init__`
- **问题**: 每次 TreeNode 实例化都重建集合，应与 OperationHistory.VALID_OPERATION_TYPES 保持 ClassVar 模式。

### TD-L2: FolderTreeService TreeNode 构造代码重复

- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) `list_root_nodes` / `_get_managed_root_node`
...
- **位置**: [folder_tree_service.py](../src/application/folder_tree_service.py) 第 336 行
- **问题**: `from pathlib import PurePath` 在函数体内 import，违反 PEP 8。

### TD-L4: UI 文本硬编码未放入 ui_constants.py

- **位置**: [main_window.py](../src/app/main_window.py) 多处
...
- **位置**: [folder_tree_model.py](../src/app/folder_tree_model.py) 第 131 行
...
- **位置**: [logging_setup.py](../src/app/logging_setup.py) 第 3 行
...
- **位置**: [main_window.py](../src/app/main_window.py) 第 692、709-710 行
...
- **位置**: [file_classify.py](../src/infrastructure/file_classify.py)
...
- **位置**: [test_migrations.py](../tests/test_migrations.py) 第 491、521、529 行
- **问题**: 与 test_db.py 的 fixture 体系不一致，缺少类型标注。

### TD-L17: test_managed_root_service.py 用 __import__ hack ✅ 已修复（Stage 5 Code Review）

- **位置**: [test_managed_root_service.py](../tests/test_managed_root_service.py) 第 352、366 行
...
- **位置**: [main_window.py](../src/app/main_window.py)
...
- **位置**: [mod_group_service.py](../src/application/mod_group_service.py) `_resolve_parent_id_by_path` / [quick_insert_service.py](../src/application/quick_insert_service.py) `_resolve_parent_id_by_path` / `_delete_folder_cache_by_path` / `_create_folder_cache_for_new_path` / `_update_parent_mtime` / [assembly_service.py](../src/application/assembly_service.py) `_sync_folder_mtime`
...
- **修复（Stage 4.5）**: 新建 [FolderCacheSyncHelper](../src/infrastructure/folder_cache_sync_helper.py)，
...
- **位置**: [quick_insert_service.py](../src/application/quick_insert_service.py) `_delete_folder_cache_by_path` / `_create_folder_cache_for_new_path` / `_resolve_parent_id_by_path` / `_update_parent_mtime`
...
- **位置**: [assembly_service.py](../src/application/assembly_service.py) `rename_as_cover`
...
- **位置**: [content_service.py](../src/application/content_service.py) / [scan_service.py](../src/application/scan_service.py) / [quick_insert_service.py](../src/application/quick_insert_service.py) / [mod_group_service.py](../src/application/mod_group_service.py) / [assembly_service.py](../src/application/assembly_service.py)
...
- **位置**: [assembly_service.py](../src/application/assembly_service.py) `_sync_folder_mtime`
...
- **位置**: [file_operation_service.py](../src/infrastructure/file_operation_service.py) `new_folder` / `move`
...
- **位置**: [db.py](../src/infrastructure/db.py) `content_unit` 表 schema
...
- **位置**: [file_operation_service.py](../src/infrastructure/file_operation_service.py)
...
- **位置**: [content_service.py](../src/application/content_service.py) `list_directory_entries` / [main_window.py](../src/app/main_window.py) `_refresh_content_list` 等
...
- **位置**: [main_window.py](../src/app/main_window.py) / [metadata_panel.py](../src/app/metadata_panel.py) 等
...
- **位置**: [models.py](../src/domain/models.py) `ContentUnit.status`
...
- **位置**: [undo_service.py](../src/application/undo_service.py) `undo()`
...
- **位置**: [file_operation_service.py](../src/infrastructure/file_operation_service.py) `move` / `rename` / `copy`
...
- **位置**: [undo_service.py](../src/application/undo_service.py) `_safety_check`
...
- **位置**: [undo_service.py](../src/application/undo_service.py) `undo()` 第 5 步
...
- **位置**: [file_operation_service.py](../src/infrastructure/file_operation_service.py) 覆盖模式分支
...
- **位置**: [file_operation_service.py](../src/infrastructure/file_operation_service.py) `rename` / `move`
...
- **位置**: [models.py](../src/domain/models.py) `ContentUnit.content_type`
...
- **位置**: [content_service.py](../src/application/content_service.py) `FileEntry`
...
- **位置**: [file_operation_service.py](../src/infrastructure/file_operation_service.py) `_sync_on_delete`
...
- **位置**: [clipboard_service.py](../src/application/clipboard_service.py) `ClipboardEntry`
...
- **位置**: [main_window.py](../src/app/main_window.py) / [ui_constants.py](../src/app/ui_constants.py) 多处
...
- **位置**: [models.py](../src/domain/models.py) `ContentUnit.VALID_CONTENT_TYPES`
...
- **位置**: [file_list_model.py](../src/app/file_list_model.py) `FileListModel`（中栏）/ [assembly_panel.py](../src/app/assembly_panel.py) `AssemblyListModel`（装配面板）
...
- **位置**: [assembly_panel.py](../src/app/assembly_panel.py) / [ui_constants.py](../src/app/ui_constants.py)
...
- **位置**: [ui_constants.py](../src/app/ui_constants.py) `THUMBNAIL_SIZE` / `THUMBNAIL_FORMAT` / `THUMBNAIL_FILENAME_TEMPLATE`
...
- **位置**: [assembly_service.py](../src/application/assembly_service.py) `remove_file`
...
- **位置**: [main_window.py](../src/app/main_window.py) /
  [search_dialog.py](../src/app/search_dialog.py) /
  [tag_filter.py](../src/app/tag_filter.py) /
  [metadata_panel.py](../src/app/metadata_panel.py) 等 docstring 与注释
...
- **位置**: [thumbnail_coordinator.py](../src/app/thumbnail_coordinator.py) `request_thumbnail` / [card_list_model.py](../src/app/card_list_model.py) / [main_window.py](../src/app/main_window.py)
...
- **位置**: [thumbnail_coordinator.py](../src/app/thumbnail_coordinator.py)
