...
> 阶段 1 新资产库路径以文件系统、内容单元旁的 `arlo.ini` 和可重建的运行时索引为事实来源。本文其余旧数据库、目录树和操作历史章节保留作遗留代码背景，并已在相关扫描章节标注待隔离。
...
阶段 1 的扫描边界以一个现役资产根目录为中心。扫描器首先检查根目录本身和其直接子项：普通顶层文件夹是内容单元候选，根目录下的松散文件是待处理异常，明确排除的系统/隐藏项、无法访问项和失效路径分别记录原因。扫描不得把多个管理根目录拼成一个隐式资产库。

发现过程只读取文件系统和 `arlo.ini`` 并生成结构结果；随后由 Application 层建立可丢弃的运行时索引或生成报告，不写数据库。发现阶段不得移动、复制、重命名或删除用户文件，也不得因为没有标签而跳过内容单元。内容单元内部直接保留原始文件名。

旧目录迁移不属于当前扫描架构。未来如重新立项，必须单独经过“只读扫描 → 预览与空间估算 → 用户选择复制或移动 → 执行 → 校验 → 报告”的顺序；复制模式默认保留源目录，任何失败都要在报告中区分失败、跳过和冲突。
