...
发现过程只读取文件系统并生成结构结果；随后由明确步骤登记 `ContentUnit`、同步 `FolderCache` 或生成报告，登记只写数据库。发现阶段不得移动、复制、重命名或删除用户文件，也不得因为没有标签而跳过内容单元。内容单元内部直接保留原始文件名，迁移和冲突处理由独立的预览/执行流程负责。

迁移执行必须经过“只读扫描 → 预览与空间估算 → 用户选择复制或移动 → 备份数据库 → 执行 → 校验 → 报告”的顺序。复制模式默认保留源目录；移动源文件到回收站需要单独确认。任何失败都要保留源文件并在报告中区分失败、跳过和冲突。
