...
- 当前状态：阶段 1 初始标签目录已固定为 15 个扁平内置标签；辅助标签和中文名称候选后置，作者/来源维度后置。
...
以下名称来自本地资源库的稳定分类，已用于确定阶段 1 的固定标签显示名称；本节其余内容只保留为来源和候选记录：
...
| 武器 | 保留 | 武器与武器护甲合并；具体类型只作为候选辅助标签记录。 |
...
“待整理”“输出文件”“归档目录”属于工作流或物理位置，不作为阶段 1 内容标签；“实验室”保留为候选记录，暂不加入固定目录。

### 二、N 网标签归并关系

下表只记录固定标签的名称来源和归并依据，不代表自动迁移规则，也不建立主分类与辅助标签层级。

| 固定标签 | 合并纳入的 N 网标签 | 处理意见 |
| --- | --- | --- |
| 核心框架 | Modders Resources；Utilities | 合并为一个固定标签，不保留两个近义标签。 |
...
| UI 界面 | User Interface | 保留为固定标签。 |
| 音效音乐 | Audio | 保留为固定标签。 |
| 画面渲染 | Models and Textures；Visuals and Graphics；Presets - ENB and ReShade 中的 ENB/ReShade 部分 | 合并视觉、图形和画面预设；模型与纹理不单独拆分。 |
...
| 作者 | 后置维度 | 阶段 1 不进入标签集合。 |
| 来源 | 后置维度 | 阶段 1 不进入标签集合。 |
| 武器 | 去掉泛化的“武器”标签 | 保留“动作武器”“单手”“双手”“弓”“法杖”作为候选辅助标签；阶段 1 只启用固定标签“武器”。 |
...
阶段 1 只提供以下 15 个固定内置标签。它们是平等的标签，不是分类；应用不提供标签分类、层级或标签目录编辑。

| 稳定 ID | 显示名称 |
| --- | --- |
| `core_framework` | 核心框架 |
| `patches` | 补丁兼容 |
| `character_appearance` | 角色美化 |
| `equipment` | 装备物品 |
| `weapons` | 武器 |
| `animation` | 动画动作 |
| `physics` | 物理系统 |
| `world` | 世界环境 |
| `gameplay` | 玩法系统 |
| `quests_locations` | 任务地点 |
| `npc_creatures` | NPC 与生物 |
| `ui` | UI 界面 |
| `audio` | 音效音乐 |
| `visuals` | 画面渲染 |
| `miscellaneous` | 其他杂项 |

- 内容单元可以没有标签，也可以拥有多个初始标签；
- 标签添加和移除由应用服务提供，内置目录本身只读；
- 作者、来源和候选辅助标签不属于阶段 1 标签集合；
- 标签 ID 写入 `arlo.ini`，显示名称不参与持久化业务判断。
#### 候选辅助标签与后置维度（不进入阶段 1）
...
以上 15 个固定标签已确认；辅助标签仍是候选结果，作者/来源维度后置。确认辅助标签前不修改 `tags.json`，也不执行已有资源的批量重分类。
